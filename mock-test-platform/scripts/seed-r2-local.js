 ...
 *
 * What it does:
 *   Uploads modules/rrb-group-d/data/*.json to local R2 at:
 *     bundles/rrb-group-d/{exam_id}/bank.json
 */
...
const DATA_DIR    = "modules/rrb-group-d/data";
const WRANGLER_DIR = "modules/rrb-group-d";
const BUCKET      = "mtp-exam-data-preview";
const PREFIX      = "bundles/rrb-group-d";
...
for (const file of files) {
  ...
  if (!examId) {
    console.warn(`  SKIP ${file} — missing bank_id`);
    continue;
  }

  const r2Key = `${PREFIX}/${examId}/bank.json`;
