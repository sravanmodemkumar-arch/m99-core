/**
 * RRB Group D desktop server — local HTMX server.
...
const HEAD = (title = "RRB Group D") => `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<div class="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm">
  <div class="text-center mb-6">
    ...
    <h1 class="text-2xl font-bold text-blue-900">RRB Group D</h1>
...
function pageHome(exams = []) {
  ...
  return HEAD("RRB Group D Mock Tests") + `
...
<header class="sticky top-0 z-50 bg-blue-800 text-white px-6 py-3 flex items-center justify-between shadow-lg">
  <div class="flex items-center gap-3">
    ...
    <div class="font-bold text-lg">RRB Group D</div>
  </div>
  <div class="flex items-center gap-3">
...
function pageExam() {
  ...
  return HEAD(`Q${session.currentIdx + 1} — ${session.examTitle || "RRB Group D"}`) + `
...
<div class="bg-blue-800 text-white px-4 py-2.5 flex items-center gap-3 flex-shrink-0">
  <div class="flex-1 min-w-0">
    <p class="font-bold text-sm truncate">${session.examTitle || "RRB Group D"}</p>
...
function pageResult(result) {
  ...
  return HEAD("Result — RRB Group D") + `
...
async function handle(req, res) {
  ...
  if (url === "/auth/login" && method === "POST") {
    ...
    const data = await apiCall("GET", "/rrb-gd/exams", null).catch(() => ({ exams: [] }));
  ...
  if (url === "/exam/start" && method === "POST") {
    ...
    try {
      const startData = await apiCall("POST", "/rrb-gd/exam/start", { exam_id });
      if (!startData.session_id) throw new Error("no session");

      const bundleRes = await apiCall("GET", `/rrb-gd/bundle/${startData.session_id}`, null);

      session.sessionId = startData.session_id;
      session.examId    = exam_id;
      session.examTitle = label || bundleRes.exam_title || "RRB Group D";
  ...
  if ((url === "/exam/submit" || url === "/exam/submit-auto") && method === "POST") {
    try {
      ...
      const data = await apiCall("POST", "/rrb-gd/exam/submit", {
        session_id: session.sessionId,
        responses,
        elapsed_s:  0,
      });
...
function start(assetsDir, config = {}) {
  ...
  API_BASE   = config.rrb_gd_base || "http://localhost:8792";
