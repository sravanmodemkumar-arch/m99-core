...
export default function ExamScreen({ navigation, route }) {
  const { examId, label = "RRB Group D", backRoute = "RRBGroupDHome" } = route.params || {};
  ...
  async function loadExam() {
    try {
      ...
      // Start session
      const startData = await apiPost("/rrb-gd/exam/start", { exam_id: examId });
      const sid = startData.session_id;
      durationRef.current = startData.duration_s || 3600;

      // Fetch bundle
      const bundleRes = await fetch(`${rrb_gd_base}/rrb-gd/bundle/${sid}`, {
      ...
      if (startData.resumed && startData.checkpoint) {
        ...
        const ckp = await AsyncStorage.getItem(`rrb_ckp_${sid}`);
        if (ckp) {
  ...
  async function syncProgress(sid) {
    ...
    await apiPost("/rrb-gd/exam/sync", { session_id: sid, responses: resp, elapsed_s: elapsed }).catch(() => {});
  }

  const saveCheckpoint = useCallback((sid, idx, resp) => {
    AsyncStorage.setItem(`rrb_ckp_${sid}`, JSON.stringify({ currentIdx: idx, responses: resp }));
  }, []);
  ...
  async function submitExam(autoSubmit = false) {
    ...
    try {
      ...
      const data = await apiPost("/rrb-gd/exam/submit", {
      ...
      await AsyncStorage.removeItem(`rrb_ckp_${sessionId}`);
