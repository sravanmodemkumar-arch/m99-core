 ...
 * Routes: POST /rrb-ntpc/exam/start, GET /rrb-ntpc/bundle/:sid,
 *         POST /rrb-ntpc/exam/sync, POST /rrb-ntpc/exam/submit,
 *         GET /rrb-ntpc/exams, GET /rrb-ntpc/stats, GET /rrb-ntpc/history
 *
 * KV keys:
...
async function _start(req, env, payload) {
    ...
    if (tsfRaw) {
      ...
      if (tsf.status === "active") {
        return _json({
          ...
          bundle_url: `/rrb-ntpc/bundle/${existingSid}`,
  ...
  return _json({
    session_id: sessionId,
    bundle_url: `/rrb-ntpc/bundle/${sessionId}`,
    duration_s: examConfig.duration_s,
    started_at: startedAt,
    elapsed_s:  0,
    resumed:    false,
...
export default {
  async fetch(req, env) {
    ...
    try {
      ...
      if (req.method === "POST" && path === "/rrb-ntpc/exam/start")      return _start(req, env, payload);
      if (req.method === "GET"  && path.startsWith("/rrb-ntpc/bundle/")) return _bundle(req, env, path);
      if (req.method === "POST" && path === "/rrb-ntpc/exam/sync")       return _sync(req, env, payload);
      if (req.method === "POST" && path === "/rrb-ntpc/exam/submit")     return _submit(req, env, payload);
      if (req.method === "GET"  && path === "/rrb-ntpc/exams")           return _exams(req, env, payload);
      if (req.method === "GET"  && path === "/rrb-ntpc/stats")           return _stats(req, env, payload);
      if (req.method === "GET"  && path.startsWith("/rrb-ntpc/history")) return _history(req, env, payload, url);

      return _json({ error: "Not found" }, 404);
    } catch (e) {
      console.error("[rrb-ntpc]", e);
