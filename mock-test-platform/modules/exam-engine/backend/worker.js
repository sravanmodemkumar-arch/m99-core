 ...
 * Routes: POST /exam/exam/start, GET /exam/bundle/:sid,
 *         POST /exam/exam/sync, POST /exam/exam/submit,
 *         GET /exam/exams, GET /exam/stats, GET /exam/history
 *
 * KV keys:
...
async function _start(req, env, payload) {
    ...
    if (tsfRaw) {
      ...
      if (tsf.status === "active") {
        return _json({
          ...
          bundle_url: `/exam/bundle/${existingSid}`,
  ...
  return _json({
    session_id: sessionId,
    bundle_url: `/exam/bundle/${sessionId}`,
    duration_s: examConfig.duration_s,
    started_at: startedAt,
    elapsed_s:  0,
    resumed:    false,
...
async function _bundle(req, env, path) {
  const sessionId = path.replace(/^\/exam\/bundle\//, "");
...
export default {
  async fetch(req, env) {
    ...
    try {
      ...
      if (req.method === "POST" && path === "/exam/exam/start")      return _start(req, env, payload);
      if (req.method === "GET"  && path.startsWith("/exam/bundle/")) return _bundle(req, env, path);
      if (req.method === "POST" && path === "/exam/exam/sync")       return _sync(req, env, payload);
      if (req.method === "POST" && path === "/exam/exam/submit")     return _submit(req, env, payload);
      if (req.method === "GET"  && path === "/exam/exams")           return _exams(req, env, payload);
      if (req.method === "GET"  && path === "/exam/stats")           return _stats(req, env, payload);
      if (req.method === "GET"  && path.startsWith("/exam/history")) return _history(req, env, payload, url);

      return _json({ error: "Not found" }, 404);
    } catch (e) {
      console.error("[exam-engine]", e);
