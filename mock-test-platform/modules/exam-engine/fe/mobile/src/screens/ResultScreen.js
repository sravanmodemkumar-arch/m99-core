...
export default function ResultScreen({ navigation, route }) {
  const {
    ...
    pct, elapsedS = 0, label = "Result", apiPrefix, backRoute,
  } = route.params || {};
  ...
  const answerRows = React.useMemo(() => {
    ...
    if (sections) {
      for (const [sid, s] of Object.entries(sections))
        for (const qid of Object.keys(s.responses || {})) qSec[qid] = sid;
    }
    let idx = 0;
    return Object.entries(answer_key || {}).map(([qid, correct]) => {
      ...
      return { idx, qid, sec: labels[sid] || sid, chosen: r.chosen || "—", correct, verdict, marks };
    });
  }, [data]);
  ...
  return (
    <SafeAreaView style={styles.shell}>
      ...
      <View style={styles.header}>
        <TouchableOpacity style={styles.hdrBtn} onPress={() => navigation.replace(backRoute || "Home")}>
          <Text style={styles.hdrBtnTxt}>← Home</Text>
        </TouchableOpacity>
        <Text style={styles.hdrTitle} numberOfLines={1}>{label}</Text>
        <TouchableOpacity style={[styles.hdrBtn, styles.hdrBtnPurple]} onPress={() =>
              ...
              return (
                <View key={def.id} style={[styles.secCard, { borderLeftColor: col }]}>
                  ...
                  <View style={styles.secStats}>
                    {[["✓", s.correct, "#2e7d32"], ["✗", s.wrong, "#c62828"], ["—", s.skipped, "#90a4ae"]].map(([sym, val, col]) => (
                      <View key={sym} style={styles.sstat}>
                        <Text style={[styles.sstatV, { color: col }]}>{val}</Text>
        ...
        <View style={styles.section}>
          ...
          {filtered.map(r => (
            <View key={r.qid} style={[styles.ansRow, styles[`ans_${r.verdict}`]]}>
              ...
              <Text style={styles.ansSec} numberOfLines={1}>{r.sec}</Text>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={[styles.fBtn, styles.fBtnOutline]} onPress={() => navigation.replace(backRoute || "Home")}>
          <Text style={styles.fBtnTxt}>← Home</Text>
        </TouchableOpacity>
        <Text style={styles.footerMeta}>{fmtTime(elapsedS)} · {total} Qs</Text>
        <TouchableOpacity style={[styles.fBtn, styles.fBtnPurple]} onPress={() => navigation.navigate("Analysis", route.params)}>
...
const styles = StyleSheet.create({
  ...
  body:    { flex: 1 },
  section: { margin: 12, backgroundColor: "#fff", borderRadius: 12, padding: 14, borderWidth: 1, borderColor: "#e0e4ed", shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 4, elevation: 2, marginBottom: 0, gap: 10 },
  secHead: { fontSize: 12, fontWeight: "800", color: "#1565c0", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 },
  ...
  accBadge:     { marginLeft: "auto", fontSize: 13, fontWeight: "800", color: "#1565c0" },
  secBarTrack:  { height: 4, backgroundColor: "#e8edf6", borderRadius: 2, overflow: "hidden" },
  ...
  ansNum:    { fontSize: 12, fontWeight: "800", color: "#1565c0", minWidth: 24 },
  ansSec:    { flex: 1, fontSize: 11, color: "#777", fontWeight: "600" },
