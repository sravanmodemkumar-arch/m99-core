/** JWT — sign + verify. Payload: { tenant_id, uid, phone, tier, modules } */

const ALG = "HS256";

export async function signJwt(payload, secret, expiryHours = 24) {
  const header = _b64url(JSON.stringify({ alg: ALG, typ: "JWT" }));
  const exp = Math.floor(Date.now() / 1000) + expiryHours * 3600;
  const body = _b64url(JSON.stringify({ ...payload, exp, iat: Math.floor(Date.now() / 1000) }));
  const sig = await _sign(`${header}.${body}`, secret);
